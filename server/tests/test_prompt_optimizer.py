...
@pytest.fixture
def setup_prompt_system_with_test_run(test_db, sample_prompt_system, sample_regression_set):
    ...
    try:
        ...
        for i, sample in enumerate(sample_regression_set):
            result = TestResult(
                ...
                sample_id=str(i),
                input_variables=json.dumps({k: v for k, v in sample.items() if k != 'expected_output'}),
...
def test_start_prompt_optimization_success(client, test_db, setup_prompt_system_with_test_run, mock_redis):
    ...
    with patch('main.get_improved_prompt', new_callable=AsyncMock) as mock_get_prompt:
        with patch('main.test_improved_prompt', new_callable=AsyncMock) as mock_test_prompt:
...
def test_start_prompt_optimization_no_test_runs(client, test_db, sample_prompt_system, mock_redis):
...
def test_get_optimization_status(client, test_db, setup_prompt_system_with_test_run, mock_redis):
    ...
    with patch('main.get_improved_prompt', new_callable=AsyncMock) as mock_get_prompt:
        with patch('main.test_improved_prompt', new_callable=AsyncMock) as mock_test_prompt:
            ...
            start_response = client.post("/prompt-optimizer/start", json=optimization_data)
...
def test_delete_optimization_session(client, test_db, setup_prompt_system_with_test_run, mock_redis):
